...
ACTIONS = [
    {"id": "start_stream", "name": "Spustit stream", "icon": "\U0001F534", "target": "button"},
    {"id": "stop_stream", "name": "Zastavit stream", "icon": "\u23F9", "target": "button"},
    {"id": "switch_scene", "name": "Přepnout scénu", "icon": "\U0001F3AC", "input_type": "choice", "target": "button"},
    {"id": "mute_mic", "name": "Ztlumit mikrofon", "icon": "\U0001F507", "input_type": "choice", "target": "button"},
]

_AUDIO_KIND_MARKERS = ("input_capture", "output_capture")
...
def _get_input_names() -> list:
    # OBS "vstupy" (inputs) zahrnuji i kamery, prohlizec, zachyt obrazovky atd. -
    # audio zdroje (mikrofony, systemovy zvuk) maji v "inputKind" typicky
    # "..._input_capture" nebo "..._output_capture" (wasapi na Windows,
    # coreaudio na macOS, pulse/alsa na Linuxu). Video zdroje jako
    # monitor_capture/window_capture/game_capture/dshow_input timhle
    # filtrem projdou preskoceny.
    # POZOR: tohle je heuristika podle bezne pojmenovacich konvenci OBS -
    # nemam to overene naziv proti bezicimu OBS, takze pokud ti nejaky
    # skutecny audio zdroj v seznamu chybi (napr. exoticky audio plugin
    # s jinym nazvem "kind"), dej vedet jaky presne "inputKind" ma a
    # doplnime ho do _AUDIO_KIND_MARKERS nize.
    try:
        client = _connect()
    except Exception:
        return []
    try:
        ...
        return [
            item["inputName"] for item in response.inputs
            if any(marker in item.get("inputKind", "") for marker in _AUDIO_KIND_MARKERS)
        ]
    except Exception:
        return []
    finally:
